...
CTRL_SUB_CH2 = bytes.fromhex(
    ...
    "0000000000000000000000000802138cef4555c8")
# Canonical teardown frames, re-extracted from the capture (52 bytes each).
# NOTE: the ch1 BYTES are measured off the wire, but its EFFECT is not yet
# demonstrated -- Controller sent it while ch1 was already clear, so it was a
# no-op. Until a live test shows 0x3010 ch1 actually clearing a live ch1 flow,
# treat the ch1 teardown as unproven in effect. A teardown that works on ch2 but
# not ch1 would be worse than one that fails outright: it would half-work.
CTRL_UNSUB_CH1 = bytes.fromhex(
    "2729003406a23010000002010001" + "00" * 38)
...
def test_unsubscribe_ch2_matches_controller():
    got = C._build_aes67_unsubscribe_command(2, 0x06a7)
    assert got == CTRL_UNSUB_CH2, f"\n got  {got.hex()}\n want {CTRL_UNSUB_CH2.hex()}"

def test_unsubscribe_ch1_matches_controller():
    got = C._build_aes67_unsubscribe_command(1, 0x06a2)
    assert got == CTRL_UNSUB_CH1, f"\n got  {got.hex()}\n want {CTRL_UNSUB_CH1.hex()}"
